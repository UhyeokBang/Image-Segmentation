...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.bn1 = nn.BatchNorm2d(planes)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv2d(inplanes, planes, kernel_size=3, bias=False)
        self.bn2 = nn.BatchNorm2d(planes)
        self.downsample = downsample
        self.stride = stride
...
class HighResolutionModule(nn.Module):
    ...
    def _make_fuse_layers(self):
        ...
        fuse_layers = []
        for i in range(num_branches if num_branches == 1 else num_branches - 1):
            fuse_layer = []
...
class HRNetV2(nn.Module):
    def __init__(self, num_classes):
        ...
        self.stage2, pre_stage_channels = self._make_stage(self.stage2_cfg, [256])
        ...
        self.stage3, pre_stage_channels = self._make_stage(self.stage3_cfg, pre_stage_channels)
        ...
        self.stage4, pre_stage_channels = self._make_stage(self.stage4_cfg, pre_stage_channels)

        self.final_layer = nn.Conv2d(sum(pre_stage_channels), num_classes, kernel_size=1, stride=1, padding=0)
    ...
    def _make_layer(self, block, inplanes, planes, blocks, stride=1):
        ...
        for _ in range(1, blocks): #bottleneck * 4
    ...
    def forward(self, x):
        ...
        y = self.final_layer(torch.cat(y_list, 1)) #representation head
